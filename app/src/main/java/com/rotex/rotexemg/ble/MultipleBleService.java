...
public class MultipleBleService extends Service implements Constants, BleListener{
    ...
    private List<BluetoothDevice> mScanLeDeviceList=new ArrayList<BluetoothDevice>();
...
//            mBluetoothAdapter.getBluetoothLeScanner().stopScan(mLeScanCallback);
     ...
     * @return
     */
    public boolean enableTXNotification(String address) {
		/*
        ...
        BluetoothGattService RxService = mBluetoothGattMap.get(address)
                .getService(UartService.RX_SERVICE_UUID);
        if (RxService == null) {
        ...
        BluetoothGattCharacteristic TxChar = RxService
                .getCharacteristic(UartService.TX_CHAR_UUID);
        if (TxChar == null) {
            showMessage("Tx charateristic not found!");
            //  broadcastUpdate(DEVICE_DOES_NOT_SUPPORT_UART);
            return false;
        }
        boolean Notification = mBluetoothGattMap.get(address).setCharacteristicNotification(
                TxChar, true);
        ...
        BluetoothGattDescriptor descriptor = TxChar.getDescriptor(UartService.CCCD);

        boolean descriptor_boolean = descriptor
                .setValue(BluetoothGattDescriptor.ENABLE_NOTIFICATION_VALUE);

        if (!descriptor_boolean) {

            return false;
        }

        return mBluetoothGattMap.get(address).writeDescriptor(descriptor);
    ...
    public void setCharacteristicNotification(String address, String serviceUUID,
        ...
        descriptor.setValue(enabled ? BluetoothGattDescriptor.ENABLE_NOTIFICATION_VALUE :
                BluetoothGattDescriptor.DISABLE_NOTIFICATION_VALUE);
      boolean a=  mBluetoothGattMap.get(address).writeDescriptor(descriptor);
       // Log.e("setCharacteristicNotification", "setCharacteristicNotification=" + a);
...
//            Log.i(TAG, "mScanLeDeviceList.contains(device): " + mScanLeDeviceList.contains(device));
            ...
            broadcastUpdate(Constants.ACTION_BLUETOOTH_DEVICE, device);
        }
    };
            ...
            mScanCallback = new ScanCallback() {
                ...
                public void onScanResult(int callbackType, ScanResult result) {
                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                        ...
                        broadcastUpdate(Constants.ACTION_BLUETOOTH_DEVICE, result.getDevice());
    ...
    private final BluetoothGattCallback mGattCallback = new BluetoothGattCallback() {
        ...
        public void onConnectionStateChange(BluetoothGatt gatt, int status, int newState) {
            ...
            if (newState == BluetoothProfile.STATE_DISCONNECTED) {
                ...
                intentAction = Constants.ACTION_GATT_DISCONNECTING;
        ...
        public void onServicesDiscovered(BluetoothGatt gatt, int status) {
            ...
            if (status == BluetoothGatt.GATT_SUCCESS) {
                broadcastUpdate(Constants.ACTION_GATT_SERVICES_DISCOVERED, gatt.getDevice().getAddress());
            } else {
...
//                str=str+characteristic.getValue()[i];
//
//            }
          //  Log.e(TAG, "address: " + address + ",Write: " + str);
        }
    ...
    public int getmConnectedAddressListSize() {
        if (mConnectedAddressList == null) {
            return 0;
        }
        return mConnectedAddressList.size();
    }
}
